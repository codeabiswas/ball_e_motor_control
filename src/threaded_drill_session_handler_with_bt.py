"""
threaded_drill_session_handler_with_bt.py
---
This file contains the ThreadedDrillSessionHandler class, which controls a manual or automated drill session. This is also uses QThread, so that it can be called and handled by the GUI. It also has the Bluetooth code so that drills can be started/stopped using the Bluetooth Button.
...
class ThreadedDrillSessionHandler(QThread):
    ...
    def bt_button_click(self, bool_val):
        ...
        What to do when a Bluetooth button is clicked
        :param bool_val: Boolean value, where True denotes a button click
        """
